...
void main() {
  ...
  setUp(() {
    mockSignInAnonymously = MockSignInAnonymously();
    mockGoogleSignIn = MockGoogleSignIn();
    bloc = AuthBloc(signInAnonymously: mockSignInAnonymously, signInWithGoogle: mockGoogleSignIn);
  });
  ...
  group('Google sign in', () {
    ...
    test(
        'should emit [Empty, Loading, Error] states when sign in unsuccessful',
            () async {
          // Arrange
          when(mockGoogleSignIn.call()).thenAnswer((_) async => Left(ServerFailure()));
          final expected = [
            Empty(),
            Loading(),
            Error(message: 'sign-in-failed'),
